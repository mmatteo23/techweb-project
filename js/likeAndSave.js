...
function SaveThisArticle(username, id, saved){
    if(saved){          //sta togliendo il like
        saved=0;
        document.getElementById("saveContainer").innerHTML='<button id="saveBtn" aria-label="Press to save this article" onclick="SaveThisArticle(' + "'" +username+ "'" + ','+id+','+saved+')"><span class="material-icons md-36">bookmark_border</span></button>';
    }
    else{
        saved=1;
